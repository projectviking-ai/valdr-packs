...
5. Resolve the worktree (see Worktree Resolution)
...
Resolve the worktree before reviewing files.

1. Compute `contextRef = TASK-<taskKey>`
2. Call:
```
pm_session { action: "list", contextRef: "TASK-<task-key>" }
```
3. Choose the worktree (strict):
   - Use sessions where (`role === "executor"` or `launchReason` starts with `task:`) and `worktreePath` is set
   - **Do not** use `auditor` or `reviewer` roles
   - If multiple candidate sessions exist, choose the most recent (list order is newest-first)
4. If no session has a worktree:
   - Assume local repo at current working directory
   - Proceed with the local tree for review

Record the chosen worktree path in your review notes.
...
**Step 7: Notify Executor on Failure**

If the review outcome is `changes_requested`, notify the executor so it can act on the feedback immediately. **Do not notify on `approved`** — the verification gate handles approved reviews.

1. Resolve the executor session:
   - If the executor's session ULID was provided in your launch prompt, use it directly — regardless of session status.
   - Otherwise, discover it:
     ```
     pm_session { action: "list", contextRef: "TASK-<taskKey>" }
     ```
     Filter for executor candidates where `role === "executor"` or `launchReason` starts with `task:`.
     Prefer the most recent candidate (list order is newest-first).

   **Session status does not matter.** `pm_session input` wakes up and resumes closed/idle sessions — a `closed` session is a valid notification target. Do not skip notification because a session appears closed.

2. If no executor session is found, skip notification — the executor will check review status on its own.
...
3. Send the feedback:
   ...
   **Template:**
   ```
   Review complete for <taskKey>: changes_requested (score: <score>).

   Required changes:
   - <finding 1>
   - <finding 2>

   Please address the above and message me when ready for re-review.
   ```
...
# Review Summary
Task: <taskKey>
Worktree: <worktree-path | "local working directory (no agent sessions found)">
