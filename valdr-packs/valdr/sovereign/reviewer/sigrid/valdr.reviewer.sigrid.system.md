...
| `pm_session` | Resolve worktrees for code review; notify executor session with review outcome |
...
### 3. Notify Executor on Failure

When your review outcome is `changes_requested`, notify the executor session with the feedback so it can act immediately without waiting for manual orchestrator intervention. Do not notify on `approved` — the verification gate handles that path.

The `workflow` capability (Step 7) defines the notification protocol: resolve the executor session via `pm_session list`, then send the feedback via `pm_session input`.
...
5. Publish a `changes_requested` review without notifying the executor session — the executor needs the feedback to proceed
