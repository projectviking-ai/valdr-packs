...
| --- | --- | --- | --- |
| `valdr.agents.dependency-audit-agent.dependency-audit-scout.system` | `core` | No | Lean identity and intent routing |
| `valdr.agents.dependency-audit-agent.intake-routing` | `workflow` | Yes | Discovery, inventory, and routing |
| `valdr.agents.dependency-audit-agent.vulnerability-reachability` | `workflow` | Yes | Advisory analysis plus code-path reachability |
| `valdr.agents.dependency-audit-agent.unused-detection` | `workflow` | Yes | Declared-vs-used detection and classification |
| `valdr.agents.dependency-audit-agent.alignment-analysis` | `workflow` | Yes | Version drift and duplication analysis |
| `valdr.agents.dependency-audit-agent.compound-findings` | `workflow` | Yes | Multi-dimension prioritization |
| `valdr.agents.dependency-audit-agent.reporting-scorecard` | `validation` | Yes | Report completeness and scorecard integrity |
| `valdr.agents.dependency-audit-agent.upgrade-planning` | `integration` | Yes | Phased plan + execution handoff |
| `valdr.agents.dependency-audit-agent.audit-guardrails` | `constraints` | Yes | Read-only rules and fallback policy |
| `valdr.agents.dependency-audit-agent.npm-bun-context` | `context` | Yes | npm/Bun ecosystem nuance |
| `valdr.agents.dependency-audit-agent.cargo-context` | `context` | Yes | Cargo ecosystem nuance |
| `valdr.agents.dependency-audit-agent.jvm-context` | `context` | Yes | Maven/Gradle ecosystem nuance |
